...
import { AlertTriangle, Calendar, FileText, User } from 'lucide-vue-next';
...
const handleSubmit = () => {
    form.delete(`/dashboard/employee-plans/${props.plan.uuid}`, {
        preserveScroll: true,
        onSuccess: () => {
            toast.success(__('Employee plan deleted successfully.'));
            setTimeout(() => {
                close();
                redirect();
            }, 100);
    ...
    >
        <div class="space-y-6">
            ...
            <div class="space-y-3 rounded-lg border bg-muted/30 p-4">
                ...
                <div v-if="props.plan.employee" class="flex items-center gap-2 text-sm text-muted-foreground">
                    <User class="h-4 w-4" />
                    <span>{{ props.plan.employee.full_name }} ({{ props.plan.employee.employee_code }})</span>
                </div>
            ...
            <div class="flex items-start gap-3 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-muted-foreground">
                        {{ __('This plan will be soft-deleted and can be restored from trash.') }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-start space-x-3 rounded-lg border p-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-muted-foreground">
                        {{ __('I understand this action removes the plan.') }}
                    </p>
                </div>
            </div>
        </div>
